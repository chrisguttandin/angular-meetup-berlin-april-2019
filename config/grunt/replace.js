...
module.exports = (grunt) => {
    return {
        'csp-production': {
            files: {
                'build/angular-meetup-berlin-april-2019/browser/index.html': ['build/angular-meetup-berlin-april-2019/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: () => {
                            const html = fs.readFileSync('build/angular-meetup-berlin-april-2019/browser/index.html', 'utf8'); // eslint-disable-line node/no-sync
        ...
        'manifest': {
            files: {
                './': ['build/angular-meetup-berlin-april-2019/browser/ngsw.json']
            },
            options: {
                patterns: [
                    {
                    ...
                    },
                    {
                        ...
                        replacement: (_, filename) => {
                            return `"/angular-meetup-berlin-april-2019/browser/${filename}.html": "${computeHashOfFile(
                                `build/angular-meetup-berlin-april-2019/browser/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
        ...
        'runtime': {
            files: {
                './': ['build/angular-meetup-berlin-april-2019/browser/index.html']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, moduleAttribute) => {
                            if (moduleAttribute === undefined) {
                                return `<script>${fs.readFileSync(`build/angular-meetup-berlin-april-2019/browser/${filename}`)}</script>`; // eslint-disable-line node/no-sync
                            }

                            // eslint-disable-next-line node/no-sync
                            return `<script${moduleAttribute}>${fs.readFileSync(
                                `build/angular-meetup-berlin-april-2019/browser/${filename}`
                            )}</script>`;
                        }
                    }
                ]
