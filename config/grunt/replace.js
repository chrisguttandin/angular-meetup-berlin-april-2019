        ...
        'manifest': {
            ...
            options: {
                patterns: [ {
                    ...
                    match: /"\/angular-meetup-berlin-april-2019(?<filename>\/scripts\/main(?:-es(?:2015|5))?.[a-z0-9]+.js)":\s"[a-z0-9]+"/g,
                    ...
                    match: /"\/angular-meetup-berlin-april-2019(?<filename>\/styles\/styles\.[a-z0-9]*\.css)":\s"[a-z0-9]+"/g,
                    replacement: (_, filename) => {
                        return `"/angular-meetup-berlin-april-2019${ filename }": "${ computeHashOfFile(`build/angular-meetup-berlin-april-2019${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                    // Replace the hash value inside of the hashTable for "/(index|start).html" because it was modified before.
                    match: /"\/angular-meetup-berlin-april-2019\/(?<filename>index|start)\.html":\s"[a-z0-9]+"/g,
                    replacement: (_, filename) => {
